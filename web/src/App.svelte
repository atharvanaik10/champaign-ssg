  ...
  function draw() {
    ...
    if (!showTrails) {
      ...
      unitIds.forEach((u) => {
        ...
        if (latlngs.length > 1) {
          if (!poly) {
            poly = L.polyline(latlngs, { color, weight: 2, opacity: 0.6 })
...
<div class="mx-auto max-w-6xl space-y-4 p-4">
  ...
  <div class="grid grid-cols-1 gap-4 lg:grid-cols-[340px_1fr]">
    <aside class="relative z-[1500] space-y-4 rounded-xl border bg-white p-5 shadow-sm">
      <div>
        ...
        <label class="mt-1 block text-sm">
          <div class="flex items-center justify-between">
            ...
            <Info
              text="Path to the adjacency JSON with lat/lon, risk_factor, and
              neighbors." />
          </div>
          <input
            class="mt-1 w-full rounded-lg border p-2"
            bind:value={graphPath} />
        </label>
      </div>
      <div>
        <div
          class="mb-1 text-xs font-medium uppercase tracking-wide text-slate-500">
          Game parameters
        </div>
        <div class="grid grid-cols-2 gap-2">
          <label class="text-sm">
            <div class="flex items-center justify-between">
              <span>Alpha</span>
              <Info text="Defender reward when the attacked node is covered." />
            </div>
            <input
              type="number"
              step="0.1"
          ...
          <label class="text-sm">
            <div class="flex items-center justify-between">
              <span>Beta</span>
              <Info text="Defender loss when the attacked node is uncovered." />
            </div>
            <input
              type="number"
              step="0.1"
          ...
          <label class="text-sm">
            <div class="flex items-center justify-between">
              <span>Gamma</span>
              <Info
                text="Attacker reward when the attacked node is uncovered." />
            </div>
            <input
              type="number"
              step="0.1"
          ...
          <label class="text-sm">
            <div class="flex items-center justify-between">
              <span>Delta</span>
              <Info text="Attacker loss when the attacked node is covered." />
            </div>
            <input
              type="number"
              step="0.1"
        ...
        <label class="mt-2 block text-sm">
          <div class="flex items-center justify-between">
            ...
            <Info
              text="K: Upper bound on the total coverage allocation across nodes
              (not # of units)." />
          </div>
          <input
            type="number"
            class="mt-1 w-full rounded-lg border p-2"
            bind:value={resourceBudget} />
        </label>
      </div>
      <div>
        <div
        ...
        <div class="grid grid-cols-2 gap-2">
          <label class="text-sm">
            <div class="flex items-center justify-between">
              ...
              <Info text="Length of the simulated route (t = 0..T)." />
            </div>
            <input
              type="number"
              class="mt-1 w-full rounded-lg border p-2"
          ...
          <label class="text-sm">
            <div class="flex items-center justify-between">
              <span>Units</span>
              <Info text="Number of patrol units moving simultaneously." />
            </div>
            <input
              type="number"
              class="mt-1 w-full rounded-lg border p-2"
          ...
          <label class="text-sm">
            <div class="flex items-center justify-between">
              ...
              <Info
                text="Index in the node list where all units start (0-based)." />
            </div>
            <input
              type="number"
              class="mt-1 w-full rounded-lg border p-2"
          ...
          <label class="text-sm">
            <div class="flex items-center justify-between">
              <span>Seed</span>
              <Info text="Random seed for reproducible simulation." />
            </div>
            <input
              type="number"
              class="mt-1 w-full rounded-lg border p-2"
      ...
      <button
        class="w-full rounded-lg bg-blue-600 px-3 py-2 font-medium text-white
        hover:bg-blue-700"
        on:click={run}
      ...
      {#if status !== 'idle'}
        <div class="text-sm text-gray-600">{statusLabel()}: {message}</div>
