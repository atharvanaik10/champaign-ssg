...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="ALMA: generate a patrol schedule CSV")
...
def main() -> None:
    ...
    df, _ = generate_patrol_schedule(args.graph, game, patrol)
    out = Path(args.output)
    out.parent.mkdir(parents=True, exist_ok=True)
    df.to_csv(out, index=False)
    print(f"Saved patrol schedule to {out}")
